...
export default class Node {
	constructor(options) {
		...
		// internal
		this.level = 0;
		this.loaded = !!(this.data[props.children]);
		this.childNodes = [];
		this.loading = false;
		...
		if (props && typeof props.isLeaf !== 'undefined') {
			const isLeaf = getPropertyFromData(this, props.isLeaf);
		...
		if (this.data) {
			...
			if (store.defaultExpandAll && this.data[props.children]) {
				this.expanded = true;
			}
		} 
		...
		if (!this.data) return;
		const expandedKeys = store.expandedKeys;
		const key = store.key;
		if (key && expandedKeys && expandedKeys.indexOf(this.key) !== -1) {
